	...
	include_once("connection.php");
	{
		...
		function getPupil($conn, $isAll = false, $pupilName){
			$sql = "";
			
			if($isAll){
			$sql = "SELECT user.id,tblsection.sectionname,user.firstname,user.lastname,user.password,user.username,user.Status,user.emailaddress,user.role, user.address,user.contact,user.gender from user inner join tblsection on user.section_id=tblsection.section_id  WHERE user.role = 1";
			}else{
			$sql =  "SELECT user.id,tblsection.sectionname,user.firstname,user.lastname,user.password,user.username,user.Status,user.emailaddress,user.role, user.address,user.contact,user.gender from user inner join tblsection on user.section_id=tblsection.section_id  WHERE user.firstname LIKE '%$pupilName%' OR user.lastname LIKE '%$pupilName%' AND user.role = 2";
			}
		...
		function updatePupil($conn, $firstname, $lastname, $username, $password, $emailaddress,$address,$contact,$gender, $id){
		...
		if(isset($_POST['id'])){
			$result = updatePupil($conn, $_POST['firstname'], $_POST['lastname'], $_POST['username'], $_POST['password'],  $_POST['emailaddress'], $_POST['address'], $_POST['contact'], $_POST['gender'], $_POST['id']);
		...
		else if(isset($_GET['pupil'])){
			$pupil = $_GET['pupil'];
			
			$result = getPupil($conn, false, $pupil);
		}
			else{
			$result = getPupil($conn, true, null);
		}
...
?>
		<html>
			<head>
				<title>KiddieMDAS | admin_pupil_Directory</title>
				...
				<div class="wrapper">
					...
					<div class="content-wrapper">
						...
						<section class="content">
							...
							<center>
	<h2>View Pupil</h2><br><br>
	<form method="GET" action="admin_PupilDirectory.php">
		<div class="input-group">
	      <input type="text" name="pupil" class="form-control" placeholder="Search Pupil...">
		...
		<table class="table">
			...
			<?php 
				...
				if($result->num_rows > 0){
					while ($r=mysqli_fetch_array($result)){
						...
						echo "<td><a class='btn btn-primary viewdetail ' data-toggle='modal' data-target='#pupilModal' onclick=\"populateToModal('" . $r['firstname'] . "','" . $r['lastname'] . "','" . $r['username'] . "','" . $r['password'] . "','" . $r['emailaddress'] . "','" . $r['address'] . "','" . $r['contact'] . "',
							       '" . $r['gender'] . "','".$r['sectionname']."','" . $r['id'] . "', false)\">View Info</a>
						 <a class=\"btn btn-primary\" data-toggle=\"modal\" data-target=\"#pupilModal\" onclick=\"populateToModal('" . $r['firstname'] . "','" . $r['lastname'] . "','" . $r['username'] . "','" . $r['password'] . "','" . $r['emailaddress'] . "','" . $r['address'] . "','" . $r['contact'] . "',
							       '" . $r['gender'] . "','".$r['sectionname']."','" . $r['id'] . "', true)\">Update Info</a>";
						if ($r['Status']=='Activate') {
							echo " <a class=\"btn btn-danger\" data-toggle=\"modal\" data-target=\"#updatepupilstatus\" onclick=\"deleteModal('" . $r['id'] ."')\">Deactivate</a></td>";
						} else {
							echo " <a class=\"btn btn-primary\" data-toggle=\"modal\" data-target=\"#updatepupilstatus\" onclick=\"deleteModal('" . $r['id'] ."')\">Activate</a></td>";
						}
						$_SESSION["id"]=$row["id"];
						echo "</tr>";
					}
			...
			<!--<tr>  
				...
				<td><a class="btn btn-primary">Update</a><a class="btn btn-primary">Delete</a></td>
					...
					<!-- view modals -->
					<div class="modal fade" id="pupilModal" tabindex="-1" role="dialog"  aria-labelledby="pupil's modal">
						...
						<div class="modal-content">
						  ...
						  <div class="modal-body">
						  	<center>
									...
									<span  id="pupil_id" name="id" class="hidden"></span>
								    ...
								    <a  href="#"id="statusID"data-toggle="modal" data-target="#updatepupilsection">Change Section</a>
									<!--<br/><br>
									<br class="clear" /> 
						  ...
						  <div class="modal-footer">
						    ...
						    <button id="buttonId1" type="button" class="btn btn-primary" onclick="updatePupil()">Update</button>
					...
					<div class="modal fade" id="updatepupilsection" tabindex="-1" role="dialog"  aria-labelledby="pupil's modal">
					  <div class="modal-dialog" role="document">
						<div class="modal-content">
						  <div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						  ...
						  <div class="modal-body">
						  	<center>
					  			<form method="POST" action="">
									<span  id="pupilid" name="pupilid" class="hidden"></span>
						  ...
						  <div class="modal-footer">
							<button id="buttonId" type="button" class="btn btn-default" data-dismiss="modal">Close</button>
						    <button id="buttonId1" type="button" class="btn btn-primary" onclick="updatePupilSection()">Update</button>
...
<!-- modals xa add secret question-->
				  ...
				  <div class="modal-dialog" role="document">
					<div class="modal-content">
					  ...
					  <div class="modal-footer">
						...
						<!-- modals for delete  -->
					<div class="modal fade" id="updatepupilstatus" tabindex="-1" role="dialog" aria-labelledby="pupildelete's modal">
					  <div class="modal-dialog" role="document">
						<div class="modal-content">
						  <div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						  ...
						  <div class="modal-footer">
							<button  type="button" class="btn btn-default" data-dismiss="modal">Close</button>
							<button id="updatepupil" type="button" class="btn btn-primary">Submit</button>
						  </div>
						</div>
					  </div>
				...
				$("#updatepupil").click(function(e){
					window.location="admin_updatepupilstatus.php?pID=" + id;
				});
			}

				</script>
					 	<script>
					function viewPupil(id){
						window.location = '/admin_PupilDirectory.php?id=' + id;
					}
						...
						$.post(
							'admin_PupilDirectory.php', 
							{
								...
								if(truData.status == 'success'){
									...
									window.location="admin_PupilDirectory.php";
	...
	function updatePupilSection(){
		        var firstname,lastname,username,password,sectionname,emailaddress,address,contact,gender;
				$.post(
							'admin_PupilDirectory.php', 
							{
								firstname: $('#firstname').val(), 
